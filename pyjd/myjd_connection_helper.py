...
class MyJDConnectionHelper:
    ...
    def __refresh_direct_connections(self) -> None:
        ...
        response = self.device.connector.request_api(
                '/device/getDirectConnectionInfos', 'POST', None,
                self.__action_url())

        if response is not None and \
                'data' in response and \
                'infos' in response['data'] and \
                len(response['data']['infos']) != 0:
    ...
    def action(self, path: str, params: List = [], http_action: str = 'POST',
        ...
        if not self.__direct_connection_enabled or \
                self.__direct_connection_info is None or \
                time.time() < self.__direct_connection_cooldown:
    ...
    def __action_url(self) -> str:
        ...
        return '/t_' + self.device.connector.get_session_token() + '_' + \
